...
const music = new Audio("SONGS/ALL SONGS/munni.mp3");
...
playitList.forEach((item) => {
  item.addEventListener("click", (el) => {
    ...
    masterPlayBtn.classList.remove("fa-circle-play");
    masterPlayBtn.classList.add("fa-circle-pause");
    music.src = songs[currentSongInd].songPath;
    music.play();
    poster.src = songs[currentSongInd].cover;
    title.innerHTML = songs[currentSongInd].songName;
    downloadBtn.href = `${songs[currentSongInd].songPath}`;
...
const CheckPlayMode = () => {
  if (playMode === "repeat") {
    ...
    currentSongInd = Math.floor(Math.random() * songs.length);
