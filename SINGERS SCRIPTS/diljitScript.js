import { diljit } from "../Songs.js";

const music = new Audio("../SONGS/DILJIT SONGS/1.mp3");
...
playitList.forEach((item) => {
  item.addEventListener("click", (el) => {
    ...
    masterPlayBtn.classList.remove("fa-circle-play");
    masterPlayBtn.classList.add("fa-circle-pause");
    music.src = diljit[currentSongInd].songPath;
    music.play();
    poster.src = diljit[currentSongInd].cover;
    title.innerHTML = diljit[currentSongInd].songName;
    downloadBtn.href = `${diljit[currentSongInd].songPath}`;
...
const CheckPlayMode = () => {
  if (playMode === "repeat") {
    ...
    currentSongInd = Math.floor(Math.random() * diljit.length);
      updateStyling(currentSongInd);
  } else if (playMode === "next") {
    currentSongInd = (currentSongInd + 1) % diljit.length;
          updateStyling(currentSongInd);
  }
}
...
const playPreviousSong = () => {
  if (currentSongInd <= 0) {
    currentSongInd = diljit.length - 1;
  } else {
    currentSongInd--;
  }
  updateStyling(currentSongInd);
