import { talha } from "../Songs.js";

const music = new Audio("../SONGS/TALHA SONGS/Agency.mp3");
...
playitList.forEach((item) => {
  item.addEventListener("click", (el) => {
    ...
    masterPlayBtn.classList.remove("fa-circle-play");
    masterPlayBtn.classList.add("fa-circle-pause");
    music.src = talha[currentSongInd].songPath;
    music.play();
    poster.src = talha[currentSongInd].cover;
    title.innerHTML = talha[currentSongInd].songName;
    downloadBtn.href = `${talha[currentSongInd].songPath}`;
...
const CheckPlayMode = () => {
  if (playMode === "repeat") {
    ...
    currentSongInd = Math.floor(Math.random() * talha.length);
      updateStyling(currentSongInd);
  } else if (playMode === "next") {
    currentSongInd = (currentSongInd + 1) % talha.length;
          updateStyling(currentSongInd);
  }
}
