import { shub } from "../Songs.js";

const music = new Audio("../SONGS/SHUB SONGS/1.mp3");
...
playitList.forEach((item) => {
  item.addEventListener("click", (el) => {
    ...
    masterPlayBtn.classList.remove("fa-circle-play");
    masterPlayBtn.classList.add("fa-circle-pause");
    music.src = shub[currentSongInd].songPath;
    music.play();
    poster.src = shub[currentSongInd].cover;
    title.innerHTML = shub[currentSongInd].songName;
    downloadBtn.href = `${shub[currentSongInd].songPath}`;
...
const CheckPlayMode = () => {
  if (playMode === "repeat") {
    ...
    currentSongInd = Math.floor(Math.random() * shub.length);
      updateStyling(currentSongInd);
  } else if (playMode === "next") {
    currentSongInd = (currentSongInd + 1) % shub.length;
          updateStyling(currentSongInd);
  }
}
