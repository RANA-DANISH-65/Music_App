import { aujla } from "../Songs.js";

const music = new Audio("../SONGS/AUJLA SONGS/1.mp3");
...
playitList.forEach((item) => {
  item.addEventListener("click", (el) => {
    ...
    masterPlayBtn.classList.remove("fa-circle-play");
    masterPlayBtn.classList.add("fa-circle-pause");
    music.src = aujla[currentSongInd].songPath;
    music.play();
    poster.src = aujla[currentSongInd].cover;
    title.innerHTML = aujla[currentSongInd].songName;
    downloadBtn.href = `${aujla[currentSongInd].songPath}`;
...
const CheckPlayMode = () => {
  if (playMode === "repeat") {
    ...
    currentSongInd = Math.floor(Math.random() * aujla.length);
      updateStyling(currentSongInd);
  } else if (playMode === "next") {
    currentSongInd = (currentSongInd + 1) % aujla.length;
          updateStyling(currentSongInd);
  }
}
