import { parmesh } from "../Songs.js";

const music = new Audio("../SONGS/PARMESH SONGS/Aam_Jahe_Munde.mp3");
...
playitList.forEach((item) => {
  item.addEventListener("click", (el) => {
    ...
    masterPlayBtn.classList.remove("fa-circle-play");
    masterPlayBtn.classList.add("fa-circle-pause");
    music.src = parmesh[currentSongInd].songPath;
    music.play();
    poster.src = parmesh[currentSongInd].cover;
    title.innerHTML = parmesh[currentSongInd].songName;
    downloadBtn.href = `${parmesh[currentSongInd].songPath}`;
...
const CheckPlayMode = () => {
  if (playMode === "repeat") {
    ...
    currentSongInd = Math.floor(Math.random() * parmesh.length);
      updateStyling(currentSongInd);
  } else if (playMode === "next") {
    currentSongInd = (currentSongInd + 1) % parmesh.length;
          updateStyling(currentSongInd);
  }
}
...
const playPreviousSong = () => {
  if (currentSongInd <= 0) {
    currentSongInd = parmesh.length - 1;
  } else {
    currentSongInd--;
  }
  updateStyling(currentSongInd);
